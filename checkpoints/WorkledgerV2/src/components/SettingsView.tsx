    ...
    <div className="space-y-6 max-w-4xl mx-auto" id="settings-management-panel">
      ...
      <form onSubmit={handleSaveAll} className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          <h3 className="text-base font-bold text-brand-slate tracking-tight flex items-center gap-2 border-b border-slate-101 pb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs" id="user-profile-and-companies-setting-grid">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-slate-707 font-bold mb-2">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-808 font-semibold focus:outline-none focus:ring-2 focus:ring-brand-blue/30 text-sm"
                />
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-808 font-semibold focus:outline-none focus:ring-2 focus:ring-brand-blue/30 text-sm focus:border-brand-blue"
                >
                  {companies.map((c) => (
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-slate-808 font-medium focus:outline-none focus:ring-1 focus:ring-brand-blue text-xs"
                  />
                  <button
                    type="button"
                    onClick={() => {
              ...
              <div className="border border-slate-200 rounded-xl bg-slate-50 p-3.5 space-y-2">
                ...
                <div className="space-y-1 max-h-[110px] overflow-y-auto pr-1">
                  {companies.map((company) => (
                    <div key={company} className="flex justify-between items-center bg-white border border-slate-101 p-2 rounded-lg text-xs font-semibold">
                      <span className="truncate text-slate-707 max-w-[200px]">{company}</span>
                      {companies.length > 1 && (
                        <button
                          type="button"
                          onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-808 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue/30"
                min="1"
                max="24"
              />
              <p className="text-[10px] text-slate-500 leading-relaxed mt-2">
                Expected daily capacity for business weekdays in Denmark (typically <strong className="text-slate-707">7.4 hours</strong> per full collective agreements). Overtime is calculated relative to this benchmark.
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200 h-[48px]">
                ...
                />
                <label htmlFor="checkbox-override-settings" className="text-slate-707 font-bold select-none cursor-pointer">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-808 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue/30 font-semibold"
              />
              <p className="text-[10px] text-slate-500 leading-normal mt-2">
                The name/identifier of your primary office (e.g., <strong className="text-slate-707">{deviceLocation.capital} Office</strong>).
              </p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          <div className="flex justify-between items-center flex-wrap gap-2 border-b border-slate-101 pb-3">
            ...
            {!isAddingTaxYear ? (
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                <input
                  ...
                  className="bg-white border border-slate-200 rounded px-1.5 py-1 text-slate-808 text-[10px] w-16 font-mono focus:outline-none focus:ring-1 focus:ring-brand-blue"
                  min="1900"
                  max="2100"
                />
                <button
          ...
          <div className="space-y-4">
              ...
              {taxRates.map((rate, idx) => (
                <div key={rate.year} className="bg-slate-50 border border-slate-200 p-3 rounded-xl flex items-center gap-3 text-[10px] font-mono">
                  <div className="grid grid-cols-4 gap-2 items-center flex-1">
                    <span className="font-bold text-slate-808 text-xs">{rate.year} Tax Year:</span>
                    <div>
                      ...
                      <input
                        ...
                        className="bg-white border border-slate-200 rounded p-1 text-slate-808 w-full text-center focus:outline-none focus:ring-1 focus:ring-brand-blue"
                      />
                    </div>
                    <div>
            ...
            <p className="text-[10px] text-slate-500 leading-normal">
              Standard SKAT rules apply: Commutes below 24 km yield 0 deduction. Distances between 25-120 km yield <strong className="text-slate-707">2.28 DKK/km</strong> (2026 scales). Above 120 km defaults to <strong className="text-slate-707">1.14 DKK/km</strong>.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div>
                  <label className="block text-slate-605 mb-1 leading-none font-semibold">Location / Branch Name</label>
                  <input
                    type="text"
                    placeholder={`e.g. ${deviceLocation.majorCities.slice(0, 3).join(', ')}`}
                    value={newLocName}
                    onChange={(e) => setNewLocName(e.target.value)}
                    className="w-full bg-white border border-slate-200 rounded-lg p-2.5 text-slate-808 text-xs focus:outline-none focus:ring-1 focus:ring-brand-blue"
                  />
                </div>
                <div>
                  <label className="block text-slate-605 mb-1 leading-none font-semibold font-mono">Round-Trip Distance (km)</label>
                  <input
                    type="number"
                    ...
                    placeholder={`e.g. 60`}
                    className="w-full bg-white border border-slate-200 rounded-lg p-2.5 text-slate-808 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-brand-blue"
                    min="0"
                  />
                </div>
                <div className="flex items-center gap-2 pt-1">
                  <input
                    ...
                    className="w-4 h-4 rounded border-slate-300 text-brand-blue bg-white focus:ring-brand-blue/30 cursor-pointer"
                  />
                  <label htmlFor="loc-include-commute" className="text-slate-707 font-bold select-none cursor-pointer">
            ...
            <div className="md:col-span-2 space-y-3">
              ...
              <div className="bg-slate-50 rounded-xl border border-slate-200 divide-y divide-slate-101 overflow-hidden">
                    ...
                    <div key={loc.id} className="p-3.5 flex justify-between items-center text-[11px] bg-white">
                      <div>
                        ...
                        <div className="flex gap-3 text-slate-500 text-[10px] font-mono">
                          ...
                          <span>•</span>
                          <span className={loc.includeInCommute ? 'text-emerald-700 font-semibold' : 'text-slate-404'}>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-101 pb-3 gap-2">
            <h3 className="text-base font-bold text-brand-slate tracking-tight flex items-center gap-2">
            ...
            <button
              ...
              className="text-[10px] bg-slate-50 hover:bg-slate-101 text-slate-707 font-bold px-2.5 py-1.5 rounded-lg border border-slate-200 font-mono transition cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div>
                  <label className="block text-slate-605 mb-1 leading-none font-semibold">Holiday Name</label>
                  <input
                    type="text"
                    placeholder="e.g. Easter Extra Friday"
                    value={newHolidayName}
                    onChange={(e) => setNewHolidayName(e.target.value)}
                    className="w-full bg-white border border-slate-200 rounded-lg p-2 text-slate-808 text-xs focus:outline-none focus:ring-1 focus:ring-brand-blue"
                  />
                </div>
                <div>
                  <label className="block text-slate-605 mb-1 leading-none font-semibold font-mono">Calendar date</label>
                  <input
                    type="date"
                    value={newHolidayDate}
                    onChange={(e) => setNewHolidayDate(e.target.value)}
                    className="w-full bg-white border border-slate-200 rounded-lg p-2 text-slate-808 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-brand-blue"
                  />
                </div>
                <button
                  type="button"
              ...
              <div className="p-3.5 bg-amber-50 border border-brand-peach/15 rounded-xl text-slate-707 leading-relaxed text-[11px] space-y-1">
            ...
            <div className="md:col-span-2 space-y-3">
              ...
              <div className="bg-slate-50 rounded-xl border border-slate-200 max-h-[290px] overflow-y-auto divide-y divide-slate-101">
                {holidays.length > 0 ? (
                  holidays.map((holi) => (
                    <div key={holi.date} className="p-2.5 flex justify-between items-center text-[11px]">
                      <div>
                        ...
                        <span className="text-slate-808 font-semibold">{holi.name}</span>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center bg-white border border-slate-200 p-5 rounded-2xl gap-4 shadow-sm">
          <div>
            ...
            <span className="text-[11px] text-slate-505">Reset default workspaces or save parameters permanently.</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="bg-slate-50 hover:bg-slate-101 text-xs text-rose-650 font-bold px-4 py-2.5 rounded-xl border border-rose-200 transition flex items-center justify-center gap-1.5 cursor-pointer"
            >
